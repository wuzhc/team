...
class ImportController extends Controller
{
    ...
    {
        $total = 1000000;
        $len = strlen(self::$words);

        for ($i = 0; $i < $total; $i++) {
            $goods = new Goods();
            $rand = rand(1, 720);
            $goods->fdUserID = rand(1, 10);
            $goods->fdTitle = mb_substr(self::$words, rand(0, round($len/3)), rand(20, 50));
            $goods->fdCategoryID = rand(1, 8);
            $goods->fdStatus = 1;
            $goods->fdCreate = date('Y-m-d H:i:s', strtotime("-$rand days"));
            $goods->fdNo = 'ZC' . $rand;
            $res = $goods->save();
            echo $res ? $goods->id . ' insert success' : 'failed';
            echo "\r\n";
            if (!$res) print_r($goods->getErrors());
        }
    }
}